...
/**
 * Implements on-heap long array output stream
 */
public class LongArrayOutput implements BitOutput {
    ...
    // Java does not allow creating 64 bit masks with (1L << 64) - 1; (end result is
    // 0)
    static {
     ...
     * check input values, so if they're larger than what can fit the bits (you
     * should check this before writing),
    ...
    public void writeBits(long value, int bits) {
        if (bits <= bitsLeft) {
            ...
            bitsLeft -= bits;
            checkAndFlipByte(); // We could be at 0 bits left because of the <= condition .. would it be faster
                                // with
                                // the other one?
        } else {
    ...
    public byte[] getByteArray() {
        int fullLongs = position;
        int totalLength = (fullLongs + 1) * 8; // Including the last partial long
    
        byte[] byteArray = new byte[totalLength];
    
        // Copy the full longs into the byte array
        for (int i = 0; i < fullLongs; i++) {
            long value = this.longArray[i];
            for (int j = 0; j < 8; j++) {
                byteArray[i * 8 + j] = (byte) ((value >>> (8 * (7 - j))) & 0xFF); // Ensure unsigned byte
            }
        }
    
        // Copy the last partial long into the byte array
        long lastValue = this.lB;
        for (int j = 0; j < 8; j++) {
            byteArray[fullLongs * 8 + j] = (byte) ((lastValue >>> (8 * (7 - j))) & 0xFF); // Ensure unsigned byte
        }
